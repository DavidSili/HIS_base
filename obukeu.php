...
<title id="Timerhead">HIS baza - unos obuka</title>
...
$datpoc=date('n.Y.',strtotime($datpoc));
echo '<a href="#" onclick="izmena('.$ID.')">'.$mesto.' ('.$datpoc.')</a><br/>';
}
?>
		</div>
	</div>
...
<div class="wrap" style="position:absolute;top:32px;left:220px;width:800px">
	<div class="iur">
		...
		<input id="ydatpoc" type="text" name="datpoc" class="iud" autofocus/>
	...
	<div class="iur">
		<div class="iul">instruktori</div>
		<textarea id="yinstruktori" style="width:153px" rows="3" name="instruktori" class="iud"></textarea>
		<div style="clear:both;"></div>
	</div>
	<div class="iur">
	...
	<div>
		<div class="iul" style="text-align:center;border-bottom:solid thin #000;border-top:solid thin #000;padding-bottom:8px">
			...
			<a id="linkzasliku" href="uploader.php?tip=4&ID=<?php echo $xid; ?>" target="_blank"><img ID="imager" src="<?php
$timestamp=time();
$filename = 'upload_pic/T_4_'.$ai.'.jpg?timestamp='.$timestamp;

if (file_exists($filename)) {
    echo 'upload_pic/T_4_'.$ai.'.jpg?timestamp='.$timestamp;
} else {
...
function delform()
{
var r=confirm("Da li sigurno želite da obrišete obuku iz baze?");
if (r==true)
  {
...
function change_pic()
	{
		...
		document.getElementById("imager").src="upload_pic/T_4_"+newsrc+".jpg?timestamp=" + new Date().getTime();
	}
function novo()
	{
		...
		document.getElementById("linkzasliku").href="uploader.php?tip=4&ID="+ai;
...
function izmena(posebno)
	{
		...
		$.ajax({
			url:'upload_pic/T_4_'+posebno+'.jpg?timestamp='+d.getTime(),
			type:'HEAD',
			error: function()
			{
			...
			success: function()
			{
				$("#imager").attr("src","upload_pic/T_4_"+posebno+".jpg?timestamp="+d.getTime());
			}
		});
		document.getElementById("linkzasliku").href="uploader.php?tip=4&ID="+posebno;
		$.getJSON('ajax/iobuke.php', {posebno: posebno}, function(data) {
			$('#ydatpoc').val(data.ydatpoc);
			$('#ydatkraj').val(data.ydatkraj);
			$('#ymesto').val(data.ymesto);
			$('#ydrzava').val(data.ydrzava);
			$('#ytip').val(data.ytip);
			$('#yinstruktori').val(data.yinstruktori);
